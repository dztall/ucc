...
// xml
#include "ThirdParty/xml.h"

#include <ccr.h>

// resource files to load
#define COLLADA_FILE  "Resources/RainDropCharAnim.dae"
...
void on_GLES2_Init(int view_w, int view_h)
{
   ...
   */
    ...
    */
    ...
    // load the MDL model
    g_pCharModel = csrColladaOpen(COLLADA_FILE,
                                 &vertexFormat,
                                 &vertexCulling,
                                 &material,
                                  0,
    ...
    // add the model to the scene
    CSR_SceneItem* pSceneItem = csrSceneAddCollada(g_pScene, g_pCharModel, 1, 0);
    csrSceneAddModelMatrix(g_pScene, g_pCharModel, &g_CharMatrix);

    // load the cubemap texture
    g_ID[0].m_pKey     = &g_pCharModel->m_pMesh[0].m_Skin.m_CubeMap;
...
void on_GLES2_Render()
{
    ...
    glUniformMatrix4fv(slot, 1, 0, &g_CharMatrix.m_Table[0][0]);


    csrDebugDrawSkeletonCollada(g_pCharModel, g_pLineShader, 0, 0);
    ...
    CSR_Line boneLine;
        ...
        boneLine.m_Start.m_X = -1.0f;
        boneLine.m_Start.m_Y = -5.0f;
        boneLine.m_Start.m_Z = -2.0f;
        boneLine.m_End.m_X = 1.0f;
        boneLine.m_End.m_Y = 5.0f;
        boneLine.m_End.m_Z = -2.0f;

        glDisable(GL_DEPTH_TEST);
        csrDrawLine(&boneLine, g_pLineShader);
        glEnable(GL_DEPTH_TEST);
    */
}
//------------------------------------------------------------------------------
