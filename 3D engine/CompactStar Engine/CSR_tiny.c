 ...
 * ==> DirectX model demo --------------------------------------------------*
 ****************************************************************************
 * Description : A DirectX model showing a mesh                             *
...
void on_GLES2_Init(int view_w, int view_h)
{
    ...
    // load the X model
    g_pModel = csrXOpen(X_FILE, &vf, &vc, 0, 1, 0, 0, OnLoadTexture, OnApplySkin, OnDeleteTexture);

    // add it to the scene
    csrSceneAddX(g_pScene, g_pModel, 0, 0);
    csrSceneAddModelMatrix(g_pScene, g_pModel, &g_ModelMatrix);

    g_ModelMatrix.m_Table[0][0] = 0.1f;
    g_ModelMatrix.m_Table[1][1] = 0.1f;
    g_ModelMatrix.m_Table[2][2] = 0.1f;

    g_ModelMatrix.m_Table[3][1] = 10.0f;
}
//------------------------------------------------------------------------------
void on_GLES2_Final()
{
