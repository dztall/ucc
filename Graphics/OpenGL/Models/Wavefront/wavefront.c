...
#define WAVEFRONT_FILE "Resources/Model.obj"
...
GLuint     g_ColorSlot;
//------------------------------------------------------------------------------
void ParseWaveFrontFile(const char* pFileName)
{
    // open wavefront file
    FILE* pFile = fopen(WAVEFRONT_FILE, "rt");

    // succeeded?
    if (!pFile)
    {
        printf("ERROR - cannot open file - %s", WAVEFRONT_FILE);
        return;
    }
                ...
                if ((buffer[0] >= '0' && buffer[0] <= '9') || buffer[0] == '-')
                {
                    ...
                    if (readFace)
                    {
                        ...
                        if (ch == '\r' || ch == '\n')
                        {
                            ...
                            if (!g_pVertexes)
                                g_pVertexes =
                                        (QR_Vertex*)malloc(g_VertCount * sizeof(QR_Vertex));
                            else
                                g_pVertexes =
                                        (QR_Vertex*)realloc(g_pVertexes,
                                                            g_VertCount * sizeof(QR_Vertex));
    ...
    // close wavefront file
    fclose(pFile);

    // log file result
    printf("SUCCESS - parsed %i vertexes", g_VertCount);
}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
void GetTranslateMatrix(QR_Vector3* pT, QR_Matrix* pM)
{
    GetIdentity(pM);
...
//------------------------------------------------------------------------------
void ApplyOrtho(float maxX, float maxY, int vertical) const
{
...
void on_GLES2_Init(int view_w, int view_h)
{
    ...
    ParseWaveFrontFile(WAVEFRONT_FILE);
