/*
    ...
    downloaded from https://www.shadertoy.com/view/llSyDh
...
float hex(in vec2 p)
{
    ...
    //return max(p.x * 0.5 + p.y * 0.866025, p.x);
...
float dots(in vec2 p)
{
    ...
    // diamonds
    //return (p.x + p.y) / 1.5 + 0.035;

    // squares
    //return max(p.x, p.y) + 0.03;

    // hexagons
    //return max(p.x * 0.866025 + p.y * 0.5, p.y) + 0.01;

    // stars
    //return min((p.x + p.y) * 0.7071, max(p.x, p.y)) + 0.08;
}
...
float dfPol(vec2 p)
{
    ...
    // dodecahedron
    //return max(hex(p), hex(r2(3.14159 / 6.0) * p));

    // triangle
    //return max(abs(p.x) * 0.866025 - p.y, p.y);
}
...
float df(vec2 p, float dir)
{
    ...
    vec2 uv = vec2(p.x + 0.1, p.y); // *vec2(1, 1); // scaling
    ...
    return min(dots(uv * 4.5), dots(uv * 4.5 + 0.5)) - 0.3;
}
...
float getPolarCoord(vec2 q, float dir)
{
    ...
    // polar angle
    const float aNum = 1.0;
    float       a    = atan(q.y, q.x);

    // wrapping the polar angle
    return mod(a / 3.14159, 2.0 / aNum) - 1.0 / aNum;
}
...
void main(void)
{
    ...
    vec2 u = (gl_FragCoord.xy - iResolution.xy * 0.5) / res;

    // scaling and moving the screen coordinates
    vec2 sc = u * 4.0 + s.yx * iTime / 8.0;
    ...
    vec4 h = getHex(sc); // + s.yx * iTime / 2.0
    ...
    //float aRnd = sin(rnd * 6.283 + iTime * 1.5) * 0.5 + 0.5; // animating the random number

    #ifdef INTERLACING
        // random vec3 - used for some overlapping
        //vec3 lRnd = vec3(rnd * 14.4 + 0.81, fract(rnd * 21.3 + 0.97), fract(rnd * 7.2 + 0.63));
        vec3 lRnd = vec3(hash21(h.zw + 0.23), hash21(h.zw + 0.96), hash21(h.zw + 0.47));
    #endif

    // ut's possible to control the randomness to form some kind of repeat pattern
    //rnd = mod(h.z + h.w, 2.0);
    ...
    const float r  = 1.0;
    const float th = 0.2; // arc thickness
    ...
    q = r2(3.14159 * 2.0 / 3.0) * p - vec2(0, r) / s;
    vec3 db = vec3(q, dfPol(q));

     // arc three
    q = r2(3.14159 * 4.0 / 3.0) * p - vec2(0, r) / s;
    ...
    // Set the circle radius. For the hexagonal version, use 0.5 / 2.0, and for triangles, 0.7071 / 2.0
    q3.z -= 0.57735/2. + th / 2.0;
    ...
    q                = q3.xy;
    const float lnTh = 0.05;
    q                = abs(q);

    float arcBord = hex(q);
    //float arcBord = length(q); // change arc length to ".57735."
    //float arcBord = max(hex(q), hex(r2(3.14159 / 6.0) * q)); // change arc length to ".57735."

    // making the hexagonal arc
    float lnOuter = max(arcBord - 0.5, -(arcBord - 0.5 + lnTh)); //0.57735

    #ifdef INTERLACING
        float ln = min(lnOuter, (q.y * 0.866025 + q.x * 0.5, q.x) - lnTh);
    #else
        float ln = min(lnOuter, arcBord - lnTh);
    #endif

    // border lines to the maze border, if that makes any sense. :)
    float lnBord = ln - 0.03;
    ...
    float dMask = smoothstep(0.0, 0.015, d);
    ///////

    // producing the background with some subtle gradients
    vec3 bg =  mix(vec3(0, 0.4, 0.6), vec3(0, 0.3, 0.7), dot(sin(u * 6.0 - cos(u * 3.0)), vec2(0.4 / 2.0)) + 0.4);
    bg      = mix(bg, bg.xzy, dot(sin(u * 6.0 - cos(u * 3.0)), vec2(0.4 / 2.0)) + 0.4);
    bg      = mix(bg, bg.zxy, dot(sin(u * 3.0 + cos(u * 3.0)), vec2(0.1 / 2.0)) + 0.1);

    #ifdef INTERLACING
        // putting in background cube lines for the interlaced version
        float hLines = smoothstep(0.0, 0.02, eDist - 0.5 + 0.02);
        bg           = mix(bg, vec3(0), smoothstep(0.0, 0.02, ln) * dMask * hLines);
    #endif
    ...
    float hLines2 = smoothstep(0.0, 0.02, eDist2 - 0.5 + 0.02);

    #ifdef INTERLACING
        if (rnd > 0.5 && lRnd.x < 0.5)
            hLines2 *= smoothstep(0.0, 0.02, ln);
    ...
    float hLines3 = smoothstep(0.0, 0.02, eDist3 - 0.5 + 0.02);

    #ifdef INTERLACING
        if (rnd <= 0.5 && lRnd.x > 0.5)
            hLines3 *= smoothstep(0.0, 0.02, ln);
    ...
    float shade = max(1.25 - dot(h2.xy, h2.xy) * 2.0, 0.0);
    shade       = min(shade, max(dot(h3.xy, h3.xy) * 3.0 + 0.25, 0.0));
    bg          = mix(bg, vec3(0), (1.0 - shade) * 0.5);
    ...
    vec3 dotCol = bg.zyx * vec3(1.5, 0.4, 0.4);
    vec3 bCol   = mix(bg.zyx, bg.yyy, 0.25);
    bg          = mix(bg.yyy, bg.zyx, 0.25);

    // under the random threshold, and we draw the lines under the Truchet pattern
    #ifdef INTERLACING
        if (lRnd.x > 0.5)
        {
            bg = mix(bg, vec3(0), (1.0 - smoothstep(0.0, 0.015, lnBord)));
            bg = mix(bg, bCol, (1.0 - smoothstep(0.0, 0.015, ln)));

            // center lines
            bg = mix(bg, vec3(0), smoothstep(0.0, 0.02, eDist3 - 0.5 + 0.02) * tr);
        }
    #else
        bg = mix(bg, vec3(0), (1.0 - smoothstep(0.0, 0.015, lnBord)));
        bg = mix(bg, bCol, (1.0 - smoothstep(0.0, 0.015, ln)));
    ...
    bg = mix(bg, vec3(0), (1.0 - smoothstep(0.0, 0.07, d)) * 0.5);

    // place the Truchet field to the background, with some additional shading to give it a
    // slightly rounded, raised feel
    vec3 col = mix(bg, vec3(1) * max(-d * 3.0 + 0.7, 0.0), (1.0 - dMask) * 0.65);

    // apply the moving dot pattern to the Truchet
    //dotCol = mix(dotCol, dotCol.xzy, dot(sin(u * 3.14159 * 2.0 - cos(u.yx * 3.14159 * 2.0) * 3.14159), vec2(0.25)) + 0.5);
    col = mix(col, vec3(0), (1.0 - dMask) * (1.0 - smoothstep(0.0, 0.02, d2)));
    col = mix(col, dotCol, (1.0 - dMask) * (1.0 - smoothstep(0.0, 0.02, d2 + 0.125)));

    // Truchet border
    col = mix(col, vec3(0), 1.0 - smoothstep(0.0, 0.015, dBord));

    #ifdef INTERLACING
        // over the random threshold, and we draw the lines over the Truchet
        if (lRnd.x <= 0.5)
        {
            col = mix(col, vec3(0), (1.0 - smoothstep(0.0, 0.015, lnBord)));
            col = mix(col, bCol, (1.0 - smoothstep(0.0, 0.015, ln)));

            // center lines
            col = mix(col, vec3(0), smoothstep(0.0, 0.02, eDist2 - 0.5 + 0.02) * tr);
        }
    #endif
    ...
    /*
    if (rnd > 0.5)
        // all raised edges. Spoils the mild 3D illusion
        h3.y = -h3.y;
    */

    #ifdef INTERLACING
        float trSn = max(dMask, 1.0 - smoothstep(0.0, 0.015, lnBord)) * 0.75 + 0.25;
    #else
        float trSn = dMask * 0.75 + 0.25;
    #endif

    col = mix(col, vec3(0), trSn * (1.0 - hex(s / 2.0 + h2.xy)));
    col = mix(col, vec3(0), trSn * (1.0 - hex(s / 2.0 - h3.xy)));
    ...
    float cont = clamp(cos(hex(h.xy) * 6.283 * 12.0) * 1.5 + 1.25, 0.0, 1.0);
    col        = mix(col, vec3(0), (1.0 - smoothstep(0.0, 0.015, ln)) * (smoothstep(0.0, 0.015, d)) * (1.0 - cont) * 0.5);
    */

    // very basic hatch line effect
    float gr     = dot(col, vec3(0.299, 0.587, 0.114));
    float hatch  = (gr < 0.45) ? clamp(sin((sc.x - sc.y) * 3.14159 * 40.0) * 2.0 + 1.5, 0.0, 1.0) : 1.0;
    float hatch2 = (gr < 0.25) ? clamp(sin((sc.x + sc.y) * 3.14159 * 40.0) * 2.0 + 1.5, 0.0, 1.0) : 1.0;

    col *= min(hatch, hatch2) * 0.5 + 0.5;
    col *= clamp(sin((sc.x - sc.y) * 3.14159 * 80.0) * 1.5 + 0.75, 0.0, 1.0) * 0.25 + 1.0;

    // subtle vignette
    u    = gl_FragCoord.xy/iResolution.xy;
    col *= pow(16.0 * u.x * u.y * (1.0 - u.x) * (1.0 - u.y), 0.125) + 0.25;

    // colored varation
    //col = mix(pow(min(vec3(1.5, 1, 1) * col, 1.0), vec3(1, 3, 16)), col, pow(16.0 * u.x * u.y * (1.0 - u.x) * (1.0 - u.y), 0.25) * 0.75 + 0.25);
