...
// downloaded from https://www.shadertoy.com/view/MsXGWr
...
vec2 map(vec3 p)
{
    ...
    // arms
    vec2  h  = sdSegment(vec3(0.83, 0.15, 0.0), vec3(1.02, -0.6, -0.1), p);
    float d2 = h.x - 0.07;
    res.x    = smin(res.x, d2);
    h        = sdSegment(vec3(1.02, -0.6, -0.1), vec3(0.95, -1.2, 0.1), p);
...
void main(void)
{
    ...
    */
    ...
    gUseMin = iMouse.y > 0.0 && iMouse.x > gl_FragCoord.x;
    ...
    if (tmat.z > -0.5)
    {
        ...
        if (tmat.z < 1.5)
        {
            ...
            float f  = 0.0; //texturize(iChannel0, 0.15 * pos, nor).x * texturize(iChannel0, 0.2 * 0.25 * pos, nor).x;
            ...
            vec3 bnor = vec3(0.0, 0.0, 0.0);// = -1.0 + 2.0 * texturize(iChannel0, 4.0 * pos, nor);
        ...
        if (tmat.z < 3.5)
        {
            ...
            mate.xyz *= 0.2 + 0.8 * smoothstep(0.0, 1.0, 0.0 /*texturize(iChannel0, 0.1 * 2.1 * pos * vec3(4.0, 0.1, 4.0), nor).x*/);
        }
        else
        if (tmat.z < 4.5)
        {
        ...
        if (dif > 0.01)
            sha = softshadow(pos + 0.01 * nor, lig, 0.001, 32.0);
