...
// downloaded frm https://www.shadertoy.com/view/lty3Rt
...
mat3 m = mat3(0.0, 1.6, 1.2, -1.6, 0.72, -0.96, -1.2, -0.96, 1.28);
...
void main(void)
{
    ...
    float fade = min(1.0, time * 1.0) * min(1.0, max(0.0, 15.0 - time));
    ...
    vec3 campos = vec3(500.0, 850.0, -0.0 - cos((time - 1.4) / 2.0) * 2000.0); // moving
    ...
    if (radius < 1.8)
    {
        ...
        for (int q = 10; q > -10; --q)
        {
            ...
            float c    = (float(q) * 8.0 - campos.y) / rd.y; // cloud height
        ...
        for (int q = 0; q < 20; ++q)
        {
            ...
            float c    = (float(q) * 4.0 - campos.y) / rd.y; // cloud height
            vec3  cpos = campos + c * rd;
    ...
    col = fade * mix(col, vec3(0.3, 0.5, 0.9), 29.0 * (pow(sundot, 50.0) - pow(sundot, 60.0)) / (2.0 + 9.0 * abs(rd.y)));
        ...
        {
            // screen effect
            float c  = (col.r + col.g + col.b) * 0.3 * (0.6 + 0.3 * cos(gl_FragCoord.y * 1.2543)) + 0.1 * (noise((xy + time * 2.0) * 294.0) * noise((xy - time * 3.0) * 321.0));
            c       += max(0.0, 0.08 * sin(10.0 * time + xy.y * 7.2543));

            // flicker
            col = vec3(c, c, c) * (1.0 - 0.5 * pow(noise(vec2(time * 99.0, 0.0)), 9.0));
        }
        else
        {
            // bam
            float c = clamp(1.0 - (time - 2.5) * 6.0, 0.0, 1.0);
            col     = mix(col, vec3(1.0, 1.0, 1.0), c);
        }
    #endif
